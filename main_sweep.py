...
import scipy
import torch
from model import SEPC
from task import Task, define_dataset_config, define_tasks_config
from torch import optim
from tqdm import tqdm
from utils import stream_redirect_tqdm, wandbLogger
import wandb
...
def main():
    ...
    tasks_config = define_tasks_config(datasets_config, dataset_percentage=args.dataset_percentage, noise_rate=args.noise_rate)
    ...
    model = SEPC(pretrained_model_path=args.pretrained_model_path,
    ...
    for epoch in range(initial_epoch, num_epochs):
        ...
        with stream_redirect_tqdm() as orig_stdout:
            ...
            yield test_result_df # for wandb logger
...
if __name__ == '__main__':
    ...
    parser.add_argument("--soft_se", action="store_true", default=False, help="whether use soft label for regression")
    parser.add_argument("--noise_rate", type=float, default=0.0, help="noise_rate: 0.0, 0.1, 0.2, 0.3")

    args = parser.parse_args()
    
    # use wandb to log
    wandb.init(group=args.fine_tune_task[0].name, project='SEPC')
